...
class Test(object):
    ...
    def test(self):
        if self.env == 'linear':
            X = np.random.randn(2000, 864)
            Y = np.ones((2000, 1))
            nn = Linear(864, 1)    
            for epoch in range(100):
                for batch in range(0, 2000, 5):
                    x = X[batch:batch+5]
                    y = Y[batch:batch+5]
                    predict = nn.forward(x)            
                    loss = np.sum(np.square(predict - y)) / 2
                    delta = predict - y                
                    nn.backward(delta, 0.01)                
                    print(loss)
        elif self.env == 'conv':
            X = np.random.randn(200, 28, 28, 3)
            Y = np.ones((200,12,12,6))
            conv = Conv(kernel_shape=(5,5,3,6))
            relu = Relu()
            pool = Pool()
            for epoch in range(10):
                for batch in range(0, 200, 5):
                    x = X[batch:batch+5]
                    y = Y[batch:batch+5]
            ...
            X = np.random.randn(1000, 12, 12, 1)
            Y = np.ones((1000,1))
            conv = Conv(kernel_shape=(3,3,1,3))
            relu = Relu()
            pool = Pool()
            nn = Linear(75,1)
            ...
            for epoch in range(100):
                for batch in range(0, 1000, 5):
                    ...
                    #print(predict)         
                    loss = np.sum(np.square(predict - y)) / 2
                    print("Epoch-{}:{:.8f}".format(epoch+1,loss))
                    delta = predict - y                                     
                    delta = nn.backward(delta, lr)   
                    delta = delta.reshape(5,5,5,3)
                    delta = pool.backward(delta)
                    #delta = relu.backward(delta)
...
def train2():
    ...
    train_data.data = train_data.data.numpy().reshape(60000, 28 * 28) / 255.  # [60000,28x28]
    train_data.targets = train_data.targets.numpy()                           # [60000]
    test_data.data = test_data.data.numpy().reshape(10000, 28 * 28) / 255.    # [10000,28x28]
    ...
    for epoch in range(10):        
        ...
        for i in range(0, 60000, batch):
            X = train_data.data[i:i+batch]
            log(f, "[784:X]", X[0], flag)

            Y = train_data.targets[i:i+batch]

            predict = nn1.forward(X)
            log(f, "[500:nn]", predict[0], flag)

            predict = relu1.forward(predict)
            log(f, "[500:relu]", predict[0], flag)

            predict = nn2.forward(predict)
            log(f, "[10:nn]", predict[0], flag)
           
            # predict = relu2.forward(predict)
            # predict = nn3.forward(predict)

            predict2 = softmax.predict(predict)
            log(f, "[10:soft]", predict2[0], flag)

            loss, delta = softmax.cal_loss(predict, Y)
            log(f, "[10:delta]", delta[0], flag)

            # delta = nn3.backward(delta, lr)
            # delta = relu2.backward(delta)
  
            delta = nn2.backward(delta, lr)
            log(f, "[500:delta]", delta[0], flag)

            delta = relu1.backward(delta)    
            log(f, "[500:delta-relu]", delta[0], flag) 
         
            nn1.backward(delta, lr)
            

            print("Epoch-{}-{:05d}".format(str(epoch), i), ":", "loss:{:.4f}".format(loss))

        lr *= 0.95**(epoch+1)
        #np.savez("data2.npz", k1=conv1.k, b1=conv1.b, k2=conv2.k, b2=conv2.b, w3=nn3.W, b3=nn3.b, w4=nn4.W, b4=nn4.b, w5=nn5.W, b5=nn5.b)
