...
class AVLTree {
    ...
    public:
        ...
        int height();            // 获取树的高度
        int max(int a, int b);   // 比较两个值的大小

        void preOrder();         // 前序遍历AVL树
        void inOrder();          // 中序遍历AVL树
        void postOrder();        // 后序遍历AVL树
        
        AVLTreeNode<T>* search(T key);           // 递归实现查找AVL树中键值为key的节点
        AVLTreeNode<T>* iterativeSearch(T key);  // 非递归实现查找AVL树中键值为key的节点
  
        T minimum();   // 查找最小结点：返回最小结点的键值
        T maximum();   // 查找最大结点：返回最大结点的键值
        
        void insert(T key);  // 将结点(key为节点键值)插入到AVL树中
    ...
    private:
        ...
        AVLTreeNode<T>* search(AVLTreeNode<T>* x, T key) const;          // 递归实现查找"AVL树x"中键值为key的节点
        AVLTreeNode<T>* iterativeSearch(AVLTreeNode<T>* x, T key) const; // 非递归实现查找"AVL树x"中键值为key的节点
        ...
        AVLTreeNode<T>* insert(AVLTreeNode<T>* &tree, T key);             // 将结点z插入到AVL树tree中
        AVLTreeNode<T>* remove(AVLTreeNode<T>* &tree, AVLTreeNode<T>* z); // 删除AVL树tree中的结点z，并返回被删除的结点
